...
def create_group(mango_auth, token, group_data):
...
def add_users_in_group(group_data, group_user_id_list, group_data_response):
    ...
    print("ADDED USERS IN GROUP = " + group_user_id_list)
    return group_id
...
def update_admin_group(mangoapps_users, group_external_id, group_id):
    ...
    if(len(team_admins) > 0 and team_admins.isna() == False):
...
for index, row in df_all_groups.iterrows():
    ...
    group_data_response = create_group(mango_auth, token, group_data)
    group_id = ''
    #----- Add Users in Group
    ...
    #----- Updated Admins
    update_admin_group(df_all_users, df_all_groups, mango_auth, token, mangoapps_users, group_external_id, group_id)
