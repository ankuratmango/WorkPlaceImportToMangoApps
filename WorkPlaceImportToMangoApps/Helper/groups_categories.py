...
access_token = ""
# Other ENV Variables
data_for_how_many_days = 60
logfilepath = "logsGroup"
...
def getGroupMembers(access_token, group_id):
    endpoint = GRAPH_URL_PREFIX + str(group_id) + MEMBERS_SUFFIX + FIELDS_CONJ + MEMBER_FIELDS
    return getPagedData(access_token, endpoint, [])

def get_all_categories():
    url = "https://graph.facebook.com/community/knowledge_library_categories?limit=100"
    headers = {"Authorization": f"Bearer {access_token}"}
    all_categories = []
    logging.info('starting phase 1 of data extraction to fetch all group ids and associated metadata')
    page_number_groups = 0
...
def elt_main_categories(token, days):
    global access_token, data_for_how_many_days
    access_token = token
    data_for_how_many_days = days
    logging.info('Date Range: ' + str(start_time) + ' to ' + str(end_time))
    all_categories = get_all_categories()
    return all_categories


def elt_main(token, days):
    global access_token, data_for_how_many_days
    access_token = token
    data_for_how_many_days = days
    logging.info('Date Range: ' + str(start_time) + ' to ' + str(end_time))
