...
def exportUsers(filename, access_token, scim_url):
	...
	with open(filename, FILE_WRITE_PERMISSION, encoding='charmap', errors='ignore', newline='') as csvfile:
...
def buildExportRow(item, access_token, scim_url):
	...
	uid = scim_sdk.getUserProperty(item, scim_sdk.FIELD_ID)
	name = scim_sdk.getUserProperty(item, scim_sdk.FIELD_NAME, scim_sdk.FIELD_FORMATTED)
	...
	return [name, firstName, lastName, email,'', title, organization, phoneNumber, phoneNumber, manager, locale, location, '', startDate]
